    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Every action logged across the platform</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {audit.map((entry, i) => (
                <tr key={entry.id} className="border-t border-gray-50 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-slate-800">
                  <td className="px-4 py-3 text-gray-400 dark:text-gray-500">{i + 1}</td>
                  <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">{entry.user}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      entry.action === 'created' ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400' :
                      entry.action === 'updated' ? 'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400' :
                      'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400'
                    }`}>{entry.action}</span>
                  </td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{entry.table}</td>
                  <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{entry.record_id}</td>
