...
function ProductCard({ product }) {
  ...
  return (
    <div
      ...
      className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-200 cursor-pointer transition group"
    >
      <div className="aspect-video rounded-t-2xl overflow-hidden bg-blue-50 flex items-center justify-center">
        {firstImage ? (
          <img
      ...
      <div className="p-4">
        ...
        <h3 className="font-bold text-gray-800 text-sm mt-2 group-hover:text-blue-600 transition line-clamp-2">
          {product.title}
        </h3>
        <p className="text-blue-700 font-bold mt-1">UGX {Number(product.price).toLocaleString()}</p>
        <div className="flex items-center justify-between mt-2">
          <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">{product.condition}</span>
          <span className="text-xs text-gray-400">📍 {product.seller?.location || 'Uganda'}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">Find e-waste components from sellers across Uganda</p>
      </div>
