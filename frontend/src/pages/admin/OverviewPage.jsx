...
function StatCard({ icon, label, value, color, bg, to }) {
  return (
    <Link
      to={to}
      className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md hover:border-gray-200 transition group"
    >
      ...
      <div>
        ...
        {value === undefined || value === null ? (
          ...
          <p className="text-3xl font-bold text-gray-800 group-hover:text-blue-600 transition">
            {value}
          </p>
        )}
      </div>
...
function RoleBadge({ role }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${colors[name] || 'bg-gray-100 text-gray-600'}`}>
      {name}
    </span>
  );
}
...
export default function OverviewPage() {
  ...
  const statCards = [
    ...
    { icon: '👤', label: 'Total Admins',       value: stats?.total_admins,      bg: 'bg-orange-50', to: '/admin/admins'      },
    { icon: '🧑‍💼', label: 'Product Managers',  value: stats?.total_managers,    bg: 'bg-teal-50',   to: '/admin/users'       },
    { icon: '🛡️', label: 'Roles',              value: stats?.total_roles,       bg: 'bg-green-50',  to: '/admin/roles'       },
  ...
  const quickActions = [
    ...
    { label: 'View Permissions', to: '/admin/permissions', style: 'border border-gray-200 hover:bg-gray-50 text-gray-700' },
    { label: 'Audit Trail',      to: '/admin/audit',       style: 'border border-gray-200 hover:bg-gray-50 text-gray-700' },
    { label: 'Settings',         to: '/admin/settings',    style: 'border border-gray-200 hover:bg-gray-50 text-gray-700' },
  ];
    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
            <h3 className="font-bold text-gray-800 text-sm uppercase tracking-wide">Recent Users</h3>
            <Link to="/admin/users" className="text-blue-600 text-xs hover:underline font-medium">
              View all →
            </Link>
          </div>
      ...
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-6 text-white">
        <div className="flex flex-wrap gap-8 justify-between items-center">
          ...
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wide mb-1">Auth</p>
            <p className="font-bold">Sanctum</p>
          </div>
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wide mb-1">Roles</p>
            <p className="font-bold">Spatie</p>
          </div>
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wide mb-1">Environment</p>
            <p className="font-bold text-green-400">● Development</p>
          </div>
          <Link
            to="/admin/settings"
            className="bg-white text-gray-900 hover:bg-gray-100 px-4 py-2 rounded-xl text-sm font-semibold transition"
          >
            Settings →
          </Link>
        </div>
